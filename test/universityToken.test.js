...
describe("UniversityToken", function () {
  ...
  beforeEach(async function () {
    ...
    universityToken = await UniversityToken.deploy(owner.address); // Pass owner address
  });
  ...
  it("Should store the correct owner address", async function () {
  ...
  it("Should fail if sender doesn’t have enough balance", async function () {
    await expect(
      universityToken.connect(addr1).transfer(addr2.address, 100)
    ).to.be.reverted;
  });
